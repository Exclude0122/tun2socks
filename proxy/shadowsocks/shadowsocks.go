...
func (ss *Shadowsocks) DialContext(ctx context.Context, metadata *M.Metadata) (c net.Conn, err error) {
	...
	case "http":
		_, port, _ := net.SplitHostPort(ss.Base.Addr())
		c = obfs.NewHTTPObfs(c, ss.obfsHost, port)
	}

	c = ss.cipher.StreamConn(c)
	_, err = c.Write(base.SerializeSocksAddr(metadata))
	return
}
...
func (ss *Shadowsocks) DialUDP(*M.Metadata) (net.PacketConn, error) {
	...
	udpAddr, err := net.ResolveUDPAddr("udp", ss.Base.Addr())
	if err != nil {
		return nil, fmt.Errorf("resolve udp address %s: %w", ss.Base.Addr(), err)
	}
...
func parseShadowsocks(u *url.URL) (base.Proxy, error) {
	var (
		address            = u.Host
	...
	return NewShadowsocks(address, method, password, obfsMode, obfsHost)
}

func init() {
	proxy.RegisterProtocol(Proto, parseShadowsocks)
}
